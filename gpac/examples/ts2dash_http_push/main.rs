...
fn main() -> Result<(), ErrorCode> {
    ...
    let reframer = session.load_filter("reframer:rt=on")?;

    let rfnalu = session.load_filter("rfnalu").expect("Error loading rfnalu");

    // dst=http://127.0.0.1:8080/live/upload/playlist.mpd:hmode=push:template=segment:buf=-150:ll_part_hb=-1:seg_sync=no:segdur=1.0:cdur=0.5:llhls=sf:hlsc=true:cmaf=cmf2:dmode=dynamic:pssh=mv
    let dasher = session
        .load_filter("dasher:template=segment_16_:segdur=2.0:cdur=1.0:cmaf=cmf2:dmode=dynamic:profile=dashif.ll:utcs=http://127.0.0.1:8080/live/time/")
        .expect("Error loading file output filter");

    let manifest_fout = session
        .load_filter_destination(
            "http://127.0.0.1:8080/live/upload/playlist.mpd",
            "dual:hmode=push:template=segment_16_:segdur=2.0:cdur=1.0:cmaf=cmf2:dmode=dynamic:profile=dashif.ll:utcs=http://127.0.0.1:8080/live/time/",
        )
    ...
    reframer
        .set_source(&rfnalu)
        .expect("Error setting reframer source");
